...
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#btn-permissao">
  Alterar permissões
</button>

<!-- Modal -->
<div class="modal fade" id="btn-permissao" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  ...
  <br>
  <select name="selecionanivel" id="selecionanivel"required>
    <option value="8">Resolvedor</option>
...
<textarea id="motivo" name="motivo" cols="50" rows="10"required></textarea>
      ...
      <input class="btn btn-primary" type="submit" value="Salvar" name="permissao">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </form></div>
    <?php 
       if (isset($_POST["permissao"])) {
    $selecionanivel = $_POST['selecionanivel'];
    $motivo = $_POST['motivo'];
    $sql_motivo = $con_intra->query("INSERT INTO historico_user SET descricao = '$motivo', id_user = '$iden', acao = 'ALTERAR PERMISSÔES', nome_aten = '$usuario', ID_aten = '$IDLogin', data1 = '$hora:$minuto:$segundos'");
    $sql = $con_intra->query(" UPDATE login SET nivel = '$selecionanivel' WHERE id_login ='$iden' ");
...
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#btn-sta">
  Alterar Status
</button>

<!-- Modal -->
<div class="modal fade" id="btn-sta" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">Designar Chamado</h1>
      ...
      <form enctype="multipart/form-data" action="" method="POST" class="">
  <br>
  <select name="selecionaSTA" id="selecionaSTA"required>
...
<textarea id="motivo" name="motivo" cols="50" rows="10"required></textarea>
      ...
      <div class="modal-footer">
      <input class="btn btn-primary" type="submit" value="Salvar" name="STA-btn">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </form></div>
    <?php 
       if (isset($_POST["STA-btn"])) {
    $selecionaSTA = $_POST['selecionaSTA'];
    $motivo = $_POST['motivo'];
    $sql_motivo = $con_intra->query("INSERT INTO historico_user SET descricao = '$motivo', id_user = '$iden', acao = 'ALTERAR STATUS', nome_aten = '$usuario', ID_aten = '$IDLogin', data1 = '$hora:$minuto:$segundos'");
    $sql = $con_intra->query(" UPDATE login SET ativo = '$selecionaSTA' WHERE id_login ='$iden' ");
    echo "<meta HTTP-EQUIV='refresh' CONTENT='0'>";}else{}?>
...
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#btn-setor">
  Alterar Setor
</button>

<!-- Modal -->
<div class="modal fade" id="btn-setor" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">Designar Chamado</h1>
      ...
      <form enctype="multipart/form-data" action="" method="POST" class="">
  <br>
  <select name="selecionasetor" id="selecionasetor"required>

    <option value="">Selecione</option>
    <?php
    while($setores = $result_setor->fetch_array()){?>
      <option value="<?php echo $setores['IDsetor'] ?>" > <?php echo $setores['nome_setor'] ?></option>
    <?php
    } 
    ?>
...
<textarea id="motivo" name="motivo" cols="50" rows="10"required></textarea>
      ...
      <div class="modal-footer">
      <input class="btn btn-primary" type="submit" value="Salvar" name="altera-setor">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </form></div>
    <?php 
       if (isset($_POST["altera-setor"])) {
    $selecionasetor = $_POST['selecionasetor'];
    $motivo = $_POST['motivo'];
    $sql_motivo = $con_intra->query("INSERT INTO historico_user SET descricao = '$motivo', id_user = '$iden', acao = 'ALTERAR SETOR', nome_aten = '$usuario', ID_aten = '$IDLogin', data1 = '$hora:$minuto:$segundos'");
    $sql = $con_intra->query(" UPDATE login SET Setor_user = '$selecionasetor' WHERE id_login ='$iden' ");
    echo "<meta HTTP-EQUIV='refresh' CONTENT='0'>";}else{}?>
...
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#btn-email">
  Alterar E-mail
</button>

<!-- Modal -->
<div class="modal fade" id="btn-email" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">Designar Chamado</h1>
  ...
  <br>
  <p> 
          <label for="email_cad">informe novo e-mail</label><br>
          <input style="width: 100%;" id="email_cad" name="email_con" required="required" type="email" placeholder="contato@dominio.com"/> 
        </p>
<br>
<label>Informe o motivo </label>
<br>
<textarea id="motivo" name="motivo" cols="50" rows="10"required></textarea>
      ...
      <div class="modal-footer">
      <input class="btn btn-primary" type="submit" value="Salvar" name="alterar-email">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
      </div>
    </form></div>
    <?php 
    ...
    $motivo = $_POST['motivo'];
    $sql_motivo = $con_intra->query("INSERT INTO historico_user SET descricao = '$motivo', id_user = '$iden', acao = 'ALTERAR E-MAIL', nome_aten = '$usuario', ID_aten = '$IDLogin', data1 = '$hora:$minuto:$segundos'");
    $sql = $con_intra->query(" UPDATE login SET email = '$email_con' WHERE id_login ='$iden' ");
    echo "<meta HTTP-EQUIV='refresh' CONTENT='0'>";}else{}?>
